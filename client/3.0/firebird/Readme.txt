...
Common to all language bindings is the idea that each call to an interface is made using a pointer to a memory block comprising a null pointer followed by a pointer to a method table (the vTable). a method table comprises an Null pointer followed by an integer version number and pointers to each of the interface's methods as defined by the interface description (idl file). This approach is intended to map on to a typical object data structure in a programming language. For example, in Delphi and Free Pascal, the underlying data structure for each object instance starts with a pointer to the classes virtual method table following by each of the variables in their declaration order.
...
However, the actual interfaces and now less elegant. They are also implemented as "records" with the extended record type allowing each such record to comprise the null pointer, vTable pointer and the interface's method. However, there is no longer any means to inherit from one interface to another and hence each interface has to be fully defined with both its own and all inherited methods. Otherwise, the implementation of each method remains the same - a call via the vTable to the actual method.

The implementation classes remain as abstract classes but this time they form their own class hierarchy with no means to inherit from the "interface record". Extended records do not include the declaration of constants and OOAPI constant declarations have been mored to the corresponding implementation class.  Implicit type compatibility has also been lost and replaced by explicit type coercions. For example, IVersionCallbackImpl now includes:
...
1. where Firebird OOAPI constants are referenced - these can now only be referenced in the scope of the implementation class instead of both. Note: a class reference is sufficient

2. when there is a need for explicit coercion, typically from an implementation object to the corresponding interface.
